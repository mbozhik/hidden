...
export default function BackgroundImage({page}: {page: 'index' | 'secret'}) {
  ...
  return (
    <div className="absolute inset-0 grid h-full -z-20 place-items-center">
      <Image priority quality={100} className={cn('absolute object-contain sm:object-cover h-full sm:opacity-70', !secretPage ? ' !object-cover' : 'opacity-95')} src={secretPage ? secretBgImage : indexBgImage} alt="h1dden secret background" />
